...
import Alamofire
class PopularAFProvider: PopularProviderFilmContract {
    ...
    func fetchPopular(_ completion: @escaping (Result<[Film], ProviderError>) -> Void) {
        ...
        AF.request(request).responseDecodable(of: [Film].self, decoder: JSONDecoder()) { (response: DataResponse<[Film], AFError>) in
